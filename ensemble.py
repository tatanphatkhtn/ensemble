...
class Framework:
	...
	def __fit_with_ensemble_full_data(self, lal_data, unl_data, data_target):
		vprint = self.__print_wrapper
		...
		vprint('-----------------------------------')
		vprint('')
		train_size = min(len(unl_data), 200)
		random_slice = np.random.choice(a = unl_data.shape[0],  size  = train_size, replace = False)
		unl_data_trained = unl_data[random_slice, :] #sample without replacement
		unl_data = np.delete(unl_data, random_slice, axis = 0)
		...
		while(len(unl_data) > 0):
			...
			if(len(unl_data) > 0):
				sliceSize = min(len(unl_data), train_size - len(unl_data_trained))
				random_slice = np.random.choice(a = unl_data.shape[0],  size  = sliceSize, replace = False)
				more_trained_unl_data = unl_data[random_slice, :]
				unl_data = np.delete(unl_data, random_slice, axis = 0)		
				unl_data_trained = np.concatenate((unl_data_trained, more_trained_unl_data))
			# print 'unl_data_trained: ' + str(len(unl_data_trained))
	...
	def fit(self, lal_data, unl_data, data_target, train_size = None):	
		...
		if(self.__is_ensemble):
			if(train_size):
				...
				self.__fit_with_ensemble_full_data(lal_data, unl_data, data_target)
		else: 
